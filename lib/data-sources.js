...
async function blockscoutGetTokenInfo(address) {
  return fetchWithRetry(`${BLOCKSCOUT_BASE}/tokens/${address}`);
}

// --- ERC20 helpers ---

async function rpcERC20TotalSupply(tokenAddress, decimals = 18) {
  const result = await rpcCall('eth_call', [{ to: tokenAddress, data: '0x18160ddd' }, 'latest']);
  const raw = BigInt(result);
  const divisor = BigInt(10) ** BigInt(decimals);
  return Number(raw / divisor) + Number(raw % divisor) / Math.pow(10, decimals);
}

async function rpcERC20BalanceOf(tokenAddress, holderAddress, decimals = 18) {
  const paddedHolder = holderAddress.replace(/^0x/i, '').padStart(64, '0');
  const data = '0x70a08231' + paddedHolder;
...
function getProtocolTvlForProject(protocols, projectName, defillamaSlug) {
  if (defillamaSlug) {
    ...
    if (match) {
      // Use ?? not || so that a genuine 0 MegaETH TVL isn't replaced by total cross-chain TVL
      return match.chainTvls?.MegaETH ?? 0;
    }
  }
  // Fuzzy match by name (last resort — prefer setting defillama_slug in the DB)
  const match = protocols.find(p =>
    p.name.toLowerCase().includes(projectName.toLowerCase()) ||
    p.slug?.toLowerCase().includes(projectName.toLowerCase())
  );
  if (!match) return null;
  return match.chainTvls?.MegaETH ?? null;
}
