...
async function gatherEcosystemData() {
  ...
  const topProjects = await sql`
    SELECT d.project, d.name, d.category, pm.score, pm.classification, pm.tvl_usd, pm.tx_count,
  ...
  const recentDeployments = await sql`
    SELECT project, name, category, created_at FROM tracker_deployments ORDER BY created_at DESC LIMIT 5
  `;
...
async function gatherProjectData(query) {
  ...
  const projectRows = await sql`
    SELECT d.project, d.name, d.description, d.website, d.category, d.contract_address,
           d.defillama_slug, d.contract_verified, d.megamafia,
           pm.score, pm.classification, pm.tvl_usd, pm.tx_count, pm.balance_eth,
           pm.is_verified, pm.tx_count_delta, pm.usdm_balance,
    ...
    WHERE LOWER(d.project) LIKE ${'%' + q + '%'}
       OR LOWER(d.name) LIKE ${'%' + q + '%'}
       OR LOWER(d.defillama_slug) LIKE ${'%' + q + '%'}
...
${data.topProjects.map(p => `- ${p.name || p.project} (@${p.project}) [${p.category}] score=${p.score} class=${p.classification} tvl=${p.tvl_usd ? `$${(p.tvl_usd / 1e6).toFixed(2)}M` : 'N/A'} txs=${p.tx_count || 'N/A'} defillama=${p.defillama_slug ? 'yes' : 'no'}`).join('\n')}
...
RECENT DEPLOYMENTS:
${data.recentDeployments.map(d => `- ${d.name || d.project} (@${d.project}) [${d.category || '?'}] deployed ${d.created_at || 'unknown'}`).join('\n')}
...
async function generateProjectBrief(query) {
  ...
  const dataBlock = `
PROJECT: ${p.name || p.project} (@${p.project})
...
- USDM balance: ${p.usdm_balance ? `$${p.usdm_balance.toFixed(2)}` : 'N/A'}
