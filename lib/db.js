...
async function ensureInit() {
  ...
  // Enrichment columns added post-launch — idempotent on existing tables
  await sql`ALTER TABLE tracker_deployments ADD COLUMN IF NOT EXISTS name TEXT`;
  await sql`ALTER TABLE tracker_deployments ADD COLUMN IF NOT EXISTS description TEXT`;
  await sql`ALTER TABLE tracker_deployments ADD COLUMN IF NOT EXISTS website TEXT`;
...
async function insertDeployment({ id, project, url, tweet_text, created_at, contract_address, category, defillama_slug, megamafia = 0, name, description, website }) {
  await ensureInit();
  await sql`
    INSERT INTO tracker_deployments ${sql({
      id,
      project,
      url: url ?? null,
      ...
      defillama_slug: defillama_slug ?? null,
      megamafia: megamafia ? 1 : 0,
      name: name ?? null,
...
async function updateDeployment(id, fields) {
  await ensureInit();
  const allowed = ['contract_address', 'category', 'defillama_slug', 'contract_verified', 'megamafia', 'name', 'description', 'website'];
  const updates = {};
...
// Projects eligible for enrichment: have a contract address OR a known DeFiLlama slug
async function getDeploymentsForEnrichment() {
  await ensureInit();
  return sql`
    SELECT * FROM tracker_deployments
    WHERE contract_address IS NOT NULL
       OR (defillama_slug IS NOT NULL AND defillama_slug != '')
...
async function insertProjectMetrics({ deployment_id, tvl_usd, tx_count, tx_count_delta, balance_wei, balance_eth, is_verified, score, classification, usdm_balance, balance_eth_delta, usdm_balance_delta }) {
  await ensureInit();
  await sql`
    INSERT INTO tracker_project_metrics ${sql({
      deployment_id,
      tvl_usd: tvl_usd ?? null,
      tx_count: tx_count ?? null,
      tx_count_delta: tx_count_delta ?? null,
      balance_wei: balance_wei ?? null,
      balance_eth: balance_eth ?? null,
      is_verified: is_verified ?? null,
      score: score ?? null,
      classification: classification ?? null,
      usdm_balance: usdm_balance ?? null,
      balance_eth_delta: balance_eth_delta ?? null,
      usdm_balance_delta: usdm_balance_delta ?? null
    })}
  `;
}
...
async function insertEcosystemMetrics({ total_tvl, total_addresses, total_txs, txs_24h, avg_gas_price, avg_block_time, deployment_count }) {
  await ensureInit();
  await sql`
    INSERT INTO tracker_ecosystem_metrics ${sql({
      total_tvl: total_tvl ?? null,
      total_addresses: total_addresses ?? null,
      total_txs: total_txs ?? null,
      txs_24h: txs_24h ?? null,
      avg_gas_price: avg_gas_price ?? null,
      avg_block_time: avg_block_time ?? null,
      deployment_count: deployment_count ?? null
    })}
  `;
}
...
async function insertMilestone({ type, subject, metric, threshold, actual_value }) {
  ...
  const existing = await sql`
    ...
    WHERE type = ${type} AND metric = ${metric} AND threshold = ${threshold}
      AND ${subject == null ? sql`subject IS NULL` : sql`subject = ${subject}`}
  `;
  if (existing.length > 0) return existing[0];

  const rows = await sql`
    INSERT INTO tracker_milestones ${sql({
      type,
...
async function logAddressResolution({ deployment_id, method, query, result_address, confidence, success }) {
  await ensureInit();
  await sql`
    INSERT INTO tracker_address_resolutions ${sql({
      deployment_id,
      method,
      query: query ?? null,
      result_address: result_address ?? null,
...
async function insertStablecoinMetrics({ token_symbol, token_address, total_supply, staked_supply, circulating_supply, holder_count, transfer_count, transfer_count_delta, apps_deposited_usdm }) {
  await ensureInit();
  await sql`
    INSERT INTO tracker_stablecoin_metrics ${sql({
      token_symbol,
      token_address,
      total_supply: total_supply ?? null,
      staked_supply: staked_supply ?? null,
      circulating_supply: circulating_supply ?? null,
      holder_count: holder_count ?? null,
      transfer_count: transfer_count ?? null,
      transfer_count_delta: transfer_count_delta ?? null,
      apps_deposited_usdm: apps_deposited_usdm ?? null
    })}
  `;
}
